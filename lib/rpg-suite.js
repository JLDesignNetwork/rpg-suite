...
module.exports = {
        ...
        moreTemplates: {
          ...
          properties: {
            ...
            universalCharacterSheet: {
              title: 'Universal Character Sheet Template (.md)',
              description: 'Absolute path to a template to use across ALL systems. Overrides system-specific templates if "Force Universal Template" is enabled.',
              type: 'string',
              default: '',
              pattern: "^(.*\\.md)?$"
            },
            universalNpcSheet: {
              title: 'Universal NPC Sheet Template (.md)',
              description: 'Absolute path to an NPC template to use across ALL systems.',
              type: 'string',
              default: '',
              pattern: "^(.*\\.md)?$"
            },
            forceUniversal: {
              title: 'Force Universal Template',
              description: 'If checked, the universal templates above will be used instead of system-specific templates.',
              type: 'boolean',
              default: false
            }
          }
  ...
  activate(state) {
    ...
    this.subscriptions.add(atom.commands.add('atom-workspace', {
      ...
      'rpg-suite:gen-sheet-dnd5e': () => this.injectGenerator('sheet', { system: 'dnd5' }),
      'rpg-suite:gen-sheet-dnd4e': () => this.injectGenerator('sheet', { system: 'dnd4' }),
      'rpg-suite:gen-sheet-dnd35': () => this.injectGenerator('sheet', { system: 'dnd3.5' }),
      'rpg-suite:gen-sheet-cyberpunk': () => this.injectGenerator('sheet', { system: 'cpred' }),
      ...
      'rpg-suite:gen-sheet-coc': () => this.injectGenerator('sheet', { system: 'coc7' }),
      'rpg-suite:gen-sheet-vtm': () => this.injectGenerator('sheet', { system: 'vtm' }),
      'rpg-suite:gen-sheet-starfinder': () => this.injectGenerator('sheet', { system: 'sf' }),
      'rpg-suite:gen-npc': () => this.injectGenerator('npc'),
      'rpg-suite:gen-npc-dnd5e': () => this.injectGenerator('npc', { system: 'dnd5' }),
      'rpg-suite:gen-npc-dnd4e': () => this.injectGenerator('npc', { system: 'dnd4' }),
      'rpg-suite:gen-npc-dnd35': () => this.injectGenerator('npc', { system: 'dnd3.5' }),
      'rpg-suite:gen-npc-cyberpunk': () => this.injectGenerator('npc', { system: 'cpred' }),
      'rpg-suite:gen-npc-w40k': () => this.injectGenerator('npc', { system: 'w40k' }),
      'rpg-suite:gen-npc-rifts': () => this.injectGenerator('npc', { system: 'rifts' }),
      'rpg-suite:gen-npc-coc': () => this.injectGenerator('npc', { system: 'coc7' }),
      'rpg-suite:gen-npc-vtm': () => this.injectGenerator('npc', { system: 'vtm' }),
      'rpg-suite:gen-npc-starfinder': () => this.injectGenerator('npc', { system: 'sf' }),
  ...
  async injectGenerator(type, args = {}) {
    ...
    if (editor) {
      ...
      let prioritiesObj = {};
      let gameSys = (args.game || args.system || 'dnd5').toLowerCase();
      let customPath = '';
      const generators = require('./generators');
      ...
      if (type === 'sheet' || type === 'npc') {
        ...
        let forceUniversal = atom.config.get('rpg-suite.additionalConfiguration.moreTemplates.forceUniversal');

        if (type === 'sheet') {
          if (forceUniversal) {
            customPath = atom.config.get('rpg-suite.additionalConfiguration.moreTemplates.universalCharacterSheet');
          }
          if (!customPath) {
            if (gameSys === 'dnd5' || gameSys === 'dnd5e') customPath = atom.config.get('rpg-suite.systems.dnd.templates.customCharacterSheet');
            else if (gameSys === 'dnd4' || gameSys === 'dnd4e') customPath = atom.config.get('rpg-suite.systems.dnd4e.templates.customCharacterSheet');
            else if (gameSys === 'dnd3.5' || gameSys === 'dnd35') customPath = atom.config.get('rpg-suite.systems.dnd35.templates.customCharacterSheet');
            else if (gameSys === 'pf2' || gameSys === 'pathfinder') customPath = atom.config.get('rpg-suite.systems.pathfinder.templates.customCharacterSheet');
            else if (gameSys === 'cpred' || gameSys === 'cyberpunk' || gameSys === 'cp') customPath = atom.config.get('rpg-suite.systems.cyberpunk.templates.customCharacterSheet');
            else if (gameSys === 'coc' || gameSys === 'coc7' || gameSys === 'call-of-cthulhu' || gameSys === 'callofcthulhu') customPath = atom.config.get('rpg-suite.systems.coc.templates.customCharacterSheet');
            else if (gameSys === 'cp2020' || gameSys === 'cyberpunk-2020') customPath = atom.config.get('rpg-suite.systems.cyberpunk2020.templates.customCharacterSheet');
            ...
            else if (gameSys === 'vtm' || gameSys === 'vampire') customPath = atom.config.get('rpg-suite.systems.vtm.templates.customCharacterSheet');
            else if (gameSys === 'sf' || gameSys === 'starfinder') customPath = atom.config.get('rpg-suite.systems.starfinder.templates.customCharacterSheet');
          }
        } else {
          if (forceUniversal) {
            customPath = atom.config.get('rpg-suite.additionalConfiguration.moreTemplates.universalNpcSheet');
          }
          if (!customPath) {
            if (gameSys === 'dnd5' || gameSys === 'dnd5e') customPath = atom.config.get('rpg-suite.systems.dnd.templates.customNpcSheet');
            else if (gameSys === 'dnd4' || gameSys === 'dnd4e') customPath = atom.config.get('rpg-suite.systems.dnd4e.templates.customNpcSheet');
            else if (gameSys === 'dnd3.5' || gameSys === 'dnd35') customPath = atom.config.get('rpg-suite.systems.dnd35.templates.customNpcSheet');
            else if (gameSys === 'pf2' || gameSys === 'pathfinder') customPath = atom.config.get('rpg-suite.systems.pathfinder.templates.customNpcSheet');
            else if (gameSys === 'cpred' || gameSys === 'cyberpunk' || gameSys === 'cp') customPath = atom.config.get('rpg-suite.systems.cyberpunk.templates.customNpcSheet');
            else if (gameSys === 'coc' || gameSys === 'coc7' || gameSys === 'call-of-cthulhu' || gameSys === 'callofcthulhu') customPath = atom.config.get('rpg-suite.systems.coc.templates.customNpcSheet');
            else if (gameSys === 'cp2020' || gameSys === 'cyberpunk-2020') customPath = atom.config.get('rpg-suite.systems.cyberpunk2020.templates.customNpcSheet');
            ...
            else if (gameSys === 'vtm' || gameSys === 'vampire') customPath = atom.config.get('rpg-suite.systems.vtm.templates.customNpcSheet');
            else if (gameSys === 'sf' || gameSys === 'starfinder') customPath = atom.config.get('rpg-suite.systems.starfinder.templates.customNpcSheet');
          }
        }
            ...
            if (typeof localArgs[k] === 'string') {
              ...
              if (match) {
                ...
                if (isLeader && leaderVal !== undefined && !isNaN(leaderVal)) {
                  ...
                  localArgs[k] = crypto.randomInt(min, max + 1).toString();
                }
              }
            }
          }
  ...
  async testApiEndpoints() {
    ...
    let sysList = [
      { id: 'dnd5', urlConfig: 'systems.dnd.api.endpoint', mapConfig: 'systems.dnd.api.jsonMap', testName: 'aboleth' },
      { id: 'dnd4', urlConfig: 'systems.dnd4e.api.endpoint', mapConfig: 'systems.dnd4e.api.jsonMap', testName: 'goblin' },
      { id: 'dnd3.5', urlConfig: 'systems.dnd35.api.endpoint', mapConfig: 'systems.dnd35.api.jsonMap', testName: 'goblin' },
      { id: 'pf2', urlConfig: 'systems.pathfinder.api.endpoint', mapConfig: 'systems.pathfinder.api.jsonMap', testName: 'goblin' },
      { id: 'cpred', urlConfig: 'systems.cyberpunk.api.endpoint', mapConfig: 'systems.cyberpunk.api.jsonMap', testName: 'boosterganger' },
      { id: 'coc7', urlConfig: 'systems.coc.api.endpoint', mapConfig: 'systems.coc.api.jsonMap', testName: 'cultist' },
      { id: 'cyberpunk-2020', urlConfig: 'systems.cyberpunk2020.api.endpoint', mapConfig: 'systems.cyberpunk2020.api.jsonMap', testName: 'boosterganger' },
      ...
      { id: 'vtm', urlConfig: 'systems.vtm.api.endpoint', mapConfig: 'systems.vtm.api.jsonMap', testName: 'brujah' },
      { id: 'sf', urlConfig: 'systems.starfinder.api.endpoint', mapConfig: 'systems.starfinder.api.jsonMap', testName: 'space_goblin' }
    ];
    ...
    for (let sys of sysList) {
      ...
      if (customUrl && customUrl.trim() !== '') {
        ...
        try {
          ...
          let mapped = null;
          if (sys.id === 'dnd5' && (!customMap || customMap.trim() === '{}')) {
