...
function loadLocalDB(customDbPath) {
  ...
  if (loadBuiltIn) {
    try {
      const corePath = path.join(__dirname, '..', 'assets', 'monsters.json');
...
function searchLocal(db, system, name) {
  let sysKey = system;
  if (sysKey === 'dnd4' || sysKey === 'dnd3.5') sysKey = 'dnd5';
...
function pickRandomLocal(localDb, system, targetCR, env) {
  ...
  while (currentCR < targetCR && attempts < 50) {
    ...
    const pick = valid[crypto.randomInt(0, valid.length)];
    selected.push(pick);
    currentCR += (pick.cr || 1);
  }
...
async function generateEncounter(customDbPath, args) {
  ...
  const system = (args.game || args.system || 'dnd5').toLowerCase();
  ...
  if (args.cr && args.cr !== true) {
    ...
    let rawLevel = args.level || args.tier || args.generation || args.rank || args.age;
    if (rawLevel && rawLevel !== true) {
      const party = (args.party && args.party !== true) ? parseInt(args.party, 10) : 1;
      const level = parseInt(rawLevel, 10);
      if (!isNaN(level) && !isNaN(party)) {
  ...
  if (args.loot && args.loot !== 'false' && args.loot !== false) {
    ...
    // Basic loot calc
    const gold = Math.floor((actualCR * crypto.randomInt(10, 61)) * multi);
